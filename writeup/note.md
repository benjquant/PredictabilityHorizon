...
**Across regimes (Fig. 5).** Beyond the two-system illustration, we sweep the acrobot across initial energies ($\theta_1 \in \{0.5,\dots,3.0\}$) and add the integrable pendulum, measuring at each point *both* the gradient-gain slope and the independent Benettin $\lambda_1$ over long rollouts (horizons to $1.6\times10^4$ steps, several Lyapunov times). The two track each other across the whole continuum from $\lambda_1 \approx 0$ (pendulum, low-energy acrobot) to $\approx 1.3\,\text{s}^{-1}$ (strongly chaotic acrobot): best-fit slope $0.96$ (ideal $1$) and correlation $0.99$, with the points on the $y=x$ line within finite-time scatter (the $x$-error bars are the spread of $\lambda_1$ across trajectory windows). This promotes the law from a two-point illustration to a trend across regimes. Convergence of *both* estimators required the long horizon anticipated in §2.3 — at short horizons the weakly-chaotic points scatter, since the slope is the higher-variance functional.

**The horizon rule, demonstrated (Fig. 6).** We turn $T \lesssim 1/\lambda_1$ from an assertion into a measurement. Over an ensemble of nearby initial conditions on the chaotic acrobot, we track the signal-to-noise ratio of the analytic gradient $\nabla_{x_0}\|x_T\|^2$ — $\|\text{mean}\|$ over the mean across-ensemble deviation, geometric-mean-averaged over several base trajectories. The SNR declines steadily as the horizon approaches $T\lambda_1 \approx 1$ — falling several-fold through the horizon and by roughly an order of magnitude once it is well exceeded ($T\lambda_1 \gtrsim 2$), where it becomes small and erratic — the signature of a gradient swamped by chaotic sensitivity (variance $\sim e^{2\lambda_1 T}$). The precise crossover carries a $\log(1/\varepsilon)$ dependence on the perturbation size $\varepsilon$, so $1/\lambda_1$ is an order-of-magnitude horizon, not a sharp cliff.
